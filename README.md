...
Built with Go and the Charmbracelet stack (Bubbletea, Lip Gloss, Wish).
...
go run .
Make it yours
Everything you need to edit is in the portfolio/ folder:
...
The text fields (RoleLines, CurrentWork, etc.) are in portfolio/portfolio.go.
...
#    App names are globally unique on Fly.io, so pick something distinctive.
#    Find regions with: fly platform regions

# 5. Create the app on Fly (must match the app name in fly.toml)
