package ui

import "github.com/charmbracelet/lipgloss"

// Portfolio is the single source of truth for all content displayed in the TUI.
// To customise this portfolio, edit the fields below — no view code changes needed.
//
// ASCII art portrait: regenerate with
//
//	ascii-image-converter docs/assets/photo.jpg -W 40 -b -C
//
// Name art: regenerate with
//
//	figlet -f small "yourname"
type Config struct {
	// Portrait is the ASCII art shown in the left column.
	Portrait string

	// NameArt is the ASCII art name shown above the content area.
	NameArt string

	// RoleLines are the normal-weight intro paragraphs in the About view.
	// Each element is one paragraph; long strings wrap automatically.
	// Inline markdown supported: **bold**  *italic*
	RoleLines []string

	// CurrentWork are bold-highlighted paragraphs in the About view.
	// Each element is one paragraph; long strings wrap automatically.
	// Inline markdown supported: **bold**  *italic*
	CurrentWork []string

	// BackgroundParas are muted paragraphs (education, context, etc) in the About view.
	// Each element is one paragraph; long strings wrap automatically.
	// Rendered in grey by default.
	// Inline markdown supported: **bold**  *italic*
	BackgroundParas []string

	// Creations is the list of projects shown in the Creations view.
	Creations []Creation

	// Facts are the bullet points shown in the Fun Facts view.
	Facts []string

	// Links are the contact entries shown in the Contact view.
	Links []Link

	// AccentColor controls the highlight color used throughout the TUI:
	// nav cursor, name art, footer keys, bullet points, and selected items.
	//
	// Value is a 256-color terminal code (as a string). Pick one:
	//
	//	"39"  – bright cyan-blue  (default)
	//	"51"  – electric cyan
	//	"82"  – bright green
	//	"205" – hot pink
	//	"220" – amber/gold
	//	"141" – soft purple
	//
	// Full palette: https://en.wikipedia.org/wiki/ANSI_escape_code#8-bit
	// Preview in terminal: for i in {0..255}; do printf "\e[38;5;${i}m %3d \e[0m" $i; done
	AccentColor string

	// DimColor is the pale version of the accent used for the active nav tab
	// when the cursor has moved elsewhere. Should be a darker shade of AccentColor.
	//
	//	"68"  – muted blue  (default, pairs with "39"/"51")
	//	"64"  – muted green (pairs with "82")
	//	"162" – muted pink  (pairs with "205")
	//	"136" – muted amber (pairs with "220")
	//	"97"  – muted purple (pairs with "141")
	DimColor string
}

// accent and dim are derived from Portfolio so styles can reference them at package level.
var (
	accent = lipgloss.Color(Portfolio.AccentColor)
	dim    = lipgloss.Color(Portfolio.DimColor)
)

// Creation is a single project entry.
type Creation struct {
	Name  string
	Stack string
	Desc  string
}

// Link is a labelled contact entry.
type Link struct {
	Label string
	Value string
}

// Portfolio holds all content for the site. Edit this to customise your portfolio.
var Portfolio = Config{
	// Regenerate with: ascii-image-converter docs/assets/photo.jpeg -W 60 -b
	Portrait: `
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠟⠉⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⠻⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡿⠃⠀⠀⠀⠀⣀⣠⣤⣤⣤⣤⣤⣄⣀⠀⠀⠀⠀⠀⠈⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠁⠀⠀⠀⢀⣾⣿⣿⣿⣿⣿⣿⣿⣿⣿⡇⠀⠀⠀⠀⠀⠈⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠃⠀⠀⠀⠀⣸⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠄⠀⠀⠀⠀⠀⢸⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠀⠀⠀⠀⠀⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠄⠀⠀⠀⠀⠀⠀⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡄⠀⠀⡀⣄⢉⣉⣉⣈⣙⣿⣿⣿⠟⢁⣀⣀⠀⠀⠀⠀⠀⠀⢀⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⡀⠀⣷⣿⣏⣩⡉⢉⣹⣿⣿⣿⢰⣋⣉⢀⠀⢀⠀⠀⠀⢠⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣟⣶⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⢾⣿⣿⣿⣾⣿⡇⠀⠀⠀⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣦⢻⣿⣿⣿⣿⠇⠀⠀⣰⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣥⣍⣁⣀⣠⣿⣿⡿⠃⠀⠀⢀⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠿⠿⢿⡿⠟⠛⠛⠛⡏⠀⠀⠀⣠⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠿⠖⠂⠀⠾⠃⠀⠀⠀⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡧⠀⠀⠀⠀⣰⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣏⠙⠛⠿⠿⠛⠛⠉⠀⠀⠀⠀⠀⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣶⣤⣄⡀⠀⠀⠀⠀⠀⠀⠀⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡟⠀⠀⠀⠀⠀⠀⠀⣼⡿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣾⣶⣾⣿⣿⣴⣾⣿⡇⠈⠻⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠏⣹⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠇⠀⠀⠈⠙⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡿⠛⠁⠀⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡿⠀⠀⠀⠀⠀⠀⠈⠉⠛⠿⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠿⠛⠋⠁⠀⠀⠀⢀⣿⣿⣿⡿⠛⠉⠻⣿⣿⣿⣿⣿⣿⣿⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⠉⠛⠿⢿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠿⠛⠉⠀⠀⠀⠀⠀⠀⠀⠀⢸⣿⣿⣋⠀⠀⠀⠀⠀⢙⣿⣿⣿⣿⡏⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⠉⠛⠿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⠟⠛⠉⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⣿⣿⡿⠁⠀⠀⠀⣰⣿⣿⣿⣿⡿⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈
⣿⣿⣿⣿⣿⣿⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢻⡿⠀⠀⠀⠀⠀⣿⣿⣿⣿⡟⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⣿⣿⣿⣿⣿⡇⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢸⡇⠀⠀⠀⠀⠀⣿⣿⣿⠏⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⣿⣿⣿⣿⣿⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠘⠁⠀⠀⠀⠀⠀⣿⡿⠃⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⣿⣿⣿⣿⣿⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠟⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⣿⣿⣿⣿⡏⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⣿⣿⣿⣿⡇⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
`,
	// Regenerate with: figlet -f slant "yourname"
	NameArt: `
		   __       _ 
		  / /______(_)
		 / __/ ___/ / 
		/ /_/ /  / /  
		\__/_/  /_/   
              
`,

	AccentColor: "141",
	DimColor:    "140",

	RoleLines: []string{
		"is an ***engineer*** & ***creative*** on the internet,",
		"building cool things, documenting his 20s",
	},
	CurrentWork: []string{
		"Currently he's a ***founding engineer*** at",
		"***Materi*** where he's building the Cursor for",
		"knowledge work.",
	},
	BackgroundParas: []string{
		"Previously, Tri studied computer science at King's College London where he",
	},

	Creations: []Creation{
		{
			Name:  "Materi",
			Stack: "TypeScript, React, TipTap/ProseMirror",
			Desc:  "AI-native document editor. Live A4 pagination engine, AI editing features.",
		},
		{
			Name:  "Notchpets",
			Stack: "Electron, TypeScript, Swift, Supabase",
			Desc:  "macOS notch companion app. Pixel art pets that live in your MacBook notch.",
		},
		{
			Name:  "SSH Portfolio",
			Stack: "Go, Charmbracelet (Wish, Bubbletea, Lip Gloss)",
			Desc:  "This site. Terminal-based portfolio, ssh tri.sh.",
		},
	},

	Facts: []string{
		"Ran the Barcelona Marathon 2026",
		"Training toward sub-3:30 marathon (target: Fukuoka, Nov 2026)",
		"Currently learning Korean (intermediate, 2A level)",
		"Strong interest in geopolitics and defence tech",
		"Enjoys cooking — recently made chorizo croquettes, beef ragu, mango sticky rice",
		"Building toward starting my own company",
		"Side hobby: robotics (Raspberry Pi + Python)",
	},

	Links: []Link{
		{"GH", "github.com/tri27pham"},
		{"LI", "linkedin.com/in/tripppham"},
		{"✉", "tri_pham@outlook.com"},
		{"IG", "@triiipham"},
	},
}
